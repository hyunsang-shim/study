...
LPD3DXMESH				g_pSphere		= NULL;				// 기본 모델
LPD3DXEFFECT			g_pColorShader	= NULL;				// 컬러 쉐이더
...
// 텍스처
LPDIRECT3DTEXTURE9	gpEarthDiffuseMap = NULL;

// 프로그램 이름
const char*				gAppName		= "초간단 쉐이더 데모 프레임워크";

bool Setup_TextureMappingShader();
void Update_TextureMappingShader();
void Render_TextureMappingShader();
...
void RenderScene()
{
	...
	g_pColorShader->SetMatrix("gProjectionMatrix", &matProjection);
	g_pColorShader->SetVector("gColor", &D3DXVECTOR4(rand() % 100 / 100.0f, rand()%100 / 100.0f, rand() % 100 / 100.0f, 1.0f));
...
bool InitEverything(HWND hWnd)
{
	...
	if (!Setup_TextureMappingShader())	return false;
...
void Cleanup()
{
	...
	// 텍스처를 release 한다.
	if (gpEarthDiffuseMap)
	{
		gpEarthDiffuseMap->Release();
		gpEarthDiffuseMap = NULL;
	}
	// D3D를 release 한다.
    if(gpD3DDevice)
	{
...
bool Setup_TextureMappingShader()
{
	gpEarthDiffuseMap = LoadTexture("HOS.png");
	if (!gpEarthDiffuseMap) return false;

	gpTextureMappingShader = LoadShader("TextureShader.fx");
...
void Render_TextureMappingShader()
{
	...
	D3DXMatrixRotationZ(&matWorld, gRotationY);

	gpTextureMappingShader->SetTexture("DiffuseMap_Tex", gpEarthDiffuseMap);
	...
	gpTextureMappingShader->SetVector("gColor", &D3DXVECTOR4(rand() % 100 / 100.0f, rand() % 100 / 100.0f, rand() % 100 / 100.0f, 1.0f));
