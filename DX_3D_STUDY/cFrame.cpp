...
{
	// Wotld TM = WotldTM * Inversed Parent World TM
	m_matLocalTM = m_matWorldTM;

	if (pmatParent)
	{
		D3DXMATRIXA16 matInvParent;
		D3DXMatrixInverse(&matInvParent, 0, pmatParent); // Determinent에 0을 넣어줘도 된다....
...
void cFrame::CalcLocalT(IN int nKeyFrame, OUT D3DXMATRIXA16 & matT)
{
	...
	D3DXVECTOR3 v;
	D3DXVec3Lerp(&v, &m_vecPosTrack[nPrevIndex].v, &m_vecPosTrack[nNextIndex].v, t);		// 선형 보간
...
void cFrame::CalcLocalR(IN int nKeyFrame, OUT D3DXMATRIXA16 & matR)
{
	...
	D3DXQUATERNION q;
	D3DXQuaternionSlerp(&q, &m_vecRotTrack[nPrevIndex].q, &m_vecRotTrack[nNextIndex].q, t);	// 회전은 원형보간을 한다.
...
int cFrame::GetKeyFrame()
{
	...
	return GetTickCount() % (last - first) + first;	// GetTickCount() 내장 함수.
